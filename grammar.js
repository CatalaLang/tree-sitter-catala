...
module.exports = grammar({
  ...
  rules: {
    ...
    expression: $ =>
      ...
      (choice(
        ...
        prec.right('DOT', seq($.expression, $.DOT, $.qlident)),
        $.CARDINAL,
        $.DECIMAL,
        $.MONEY,
        seq($.LBRACE,
            ...
            $.RBRACE),
        //FIXME conflict prec.right('CONTENT',seq($.quident, optional(seq($.CONTENT, $.expression)))),
        prec.right('CONTENT', seq($.quident, $.LBRACE, repeat1($.struct_content_field), $.RBRACE)),
        prec.right('apply',
                   seq($.expression, $.OF, $.expression, repeat(seq($.COMMA, $.expression)))),
        prec.right('apply',
                   seq($.OUTPUT, $.OF, $.quident,
                       optional(seq($.WITH_V, $.LBRACE, repeat($.struct_content_field), $.RBRACE)))),
        prec.right('apply', seq($.expression, $.WITH, $.quident, optional(seq($.OF, $.LIDENT)))),
        prec.right('apply', seq($.expression, $.CONTAINS, $.expression)),
        prec.right('apply', seq($.SUM, $.primitive_typ, $.OF, $.expression)),
        prec.right('apply', seq($.expression, $.FOR, $.LIDENT, $.AMONG, $.expression)),
        prec.right('apply', seq(choice($.MINIMUM, $.MAXIMUM), $.OF, $.expression,
                                prec('apply',$.OR),
                                prec('apply',$.IF),
                                $.COLLECTION, $.EMPTY, $.THEN, $.expression)),
        prec.right('unop_expr', seq($.unop, $.expression)),
        prec.right('AND', seq($.expression, $.binop, $.expression)),
        prec.right('let_expr', seq($.EXISTS, $.LIDENT, $.AMONG, $.expression,
            $.SUCH, $.THAT, $.expression)),
        prec.right('let_expr', seq($.FOR, $.ALL, $.LIDENT, $.AMONG, $.expression,
            $.WE_HAVE, $.expression)),
        seq($.MATCH, $.expression, $.WITH,
            repeat1(
              seq($.ALT,
                  ...
                  $.COLON,
                  $.expression))),
        prec.right('let_expr', seq($.IF, $.expression, $.THEN, $.expression, $.ELSE, $.expression)),
        prec.right('let_expr', seq($.LET, $.LIDENT, $.DEFINED_AS, $.expression, $.IN, $.expression)),
        prec.right('top_expr', seq($.LIDENT, $.AMONG, $.expression, $.SUCH, $.THAT, $.expression)),
        prec.right('apply', seq($.expression, $.FOR, $.LIDENT, $.AMONG, $.expression,
            $.SUCH, $.THAT, $.expression)),
        prec.right('let_expr', seq($.LIDENT, $.AMONG, $.expression,
  ...
  OF: $ => prec.right('apply', token(tokens.OF)),
  COLLECTION: $ => token(tokens.COLLECTION),
  CONTAINS: $ => prec.right('apply', token(tokens.CONTAINS)),
  ...
  WITH: $ => prec.right('apply', token(tokens.WITH)),
  ...
  FOR: $ => prec.right('apply', token(tokens.FOR)),
  ...
  SUCH: $ => prec.right('apply', token(tokens.SUCH)),
  THAT: $ => token(tokens.THAT),
  NOT: $ => prec('unop_expr', token(tokens.NOT)),
  MAXIMUM: $ => prec('keyword', token(tokens.MAXIMUM)),
  MINIMUM: $ => prec('keyword', token(tokens.MINIMUM)),
  IS: $ => prec.right('let_expr', token(tokens.IS)),
  ...
  ALT: $ => prec.right('ALT', token(tokens.ALT)),
  ...
  UIDENT: $ => prec('UIDENT', token(tokens.UIDENT)),
  DIRECTIVE_ARG: $ => token(tokens.DIRECTIVE_ARG),
  DOT: $ => prec.left('DOT', token(tokens.DOT)),
  COMMA: $ => prec.left('COMMA', token(tokens.COMMA)),
  ...
  LAW_TEXT: $ => prec.right(token(tokens.LAW_TEXT)),
  ...
  PLUS: $ => prec.left('PLUS',
                       token(seq(tokens.PLUS,
                                 token.immediate(tokens.OP_KIND_SUFFIX)))),
  MINUS: $ => prec.left('PLUS',
                        token(seq(tokens.MINUS,
                                  token.immediate(tokens.OP_KIND_SUFFIX)))),
  MULT: $ => prec.left('MULT',
                       token(seq(tokens.MULT,
                                 token.immediate(tokens.OP_KIND_SUFFIX)))),
  DIV: $ => prec.left('MULT',
                      token(seq(tokens.DIV,
                                token.immediate(tokens.OP_KIND_SUFFIX)))),

  EQUAL: $ => prec('GREATER', token(tokens.EQUAL)),
  NOT_EQUAL: $ => prec('GREATER', token(tokens.NOT_EQUAL)),
  GREATER: $ => prec('GREATER',
                     token(seq(tokens.GREATER,
                               token.immediate(tokens.OP_KIND_SUFFIX)))),
  GREATER_EQUAL: $ => prec('GREATER',
                           token(seq(tokens.GREATER_EQUAL,
                                     token.immediate(tokens.OP_KIND_SUFFIX)))),
  LESSER: $ => prec('GREATER',
                    token(seq(tokens.LESSER,
                              token.immediate(tokens.OP_KIND_SUFFIX)))),
  LESSER_EQUAL: $ => prec('GREATER',
                          token(seq(tokens.LESSER_EQUAL,
                                    token.immediate(tokens.OP_KIND_SUFFIX)))),

  AND: $ => prec.right('AND', token(tokens.AND)),
  OR: $ => prec.right('AND', token(tokens.OR)),
  XOR: $ => prec.right('AND', token(tokens.XOR)),
  }
})
